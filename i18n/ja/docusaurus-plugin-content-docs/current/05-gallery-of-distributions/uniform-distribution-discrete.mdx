...
どの教科書でもまず間違いなく最初に挙げる確率の事例といえば、やっぱりサイコロじゃないでしょうか（[ベルヌーイ分布](bernoulli-distribution.mdx)よりも確率っぽい）。\
...
- 公正なコイン（裏表のどちらが出るかが同様に確からしいコイン）を使ったコイントスで、表を 1、裏を 0 としたもの。これは**成功確率が0.5**の[ベルヌーイ分布](bernoulli-distribution.mdx)と一致します。

- 目が1つしかない場合（2枚のコインを張り合わせて、両面表にしたコインでの表の出る確率）も考えられます（いや、面白いかじゃなくてあくまで可能性ですから！）。この分布は**退化分布**と呼ばれています。

もうひとつの特別な状況が**連続極限**で、その場合は[一様分布（連続）](uniform-distribution-continuous.mdx)となります。
