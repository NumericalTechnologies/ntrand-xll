...
それぞれの資産の価値は日々、否、時々刻々と変動しています。それぞれの資産の価値の変動は**[正規分布（単変量）](https://www.ntrand.com/jp/normal-distribution-single/)**に従うとみなされます。気になるのは資産全体の価値の変動です。はたしてどのような変動に従うのでしょうか？\
それぞれの資産価値がてんで勝手に変動するならば答えは簡単ですが、例えばドルが安くなると国内の株価が下がるなど、これらの資産はお互いに関連しあいながら変動することが分かっています。 つまり、各資産の変動を表す正規分布の情報（[平均](https://www.ntrand.com/jp/glossary/#local_mean)と[標準偏差](https://www.ntrand.com/jp/glossary/#local_standard_deviation)）に加えて、お互いにどのように関連しているかを表す情報が必要になります。それが**相関**です。
...
- 関連 NtRand 関数 : [NTBINORMDIST](https://www.ntrand.com/jp/ntbinormdist/)
- 第4引数が*TRUE*の場合の[NTBINORMDIST](https://www.ntrand.com/jp/ntbinormdist/)[NTBINROMDIST](https://www.ntrand.com/jp/ntbinormdist/)は、相関係数が $\rho=\pm 1$ かつ $x1=x2$ の場合にエラーとなります。
...
- Mersenne Twiseter 法による乱数生成 : [NTRANDMULTINORM](https://www.ntrand.com/jp/ntrandmultinorm/)
- 2変数正規分布の分布関数 : [NTBINORMDIST](https://www.ntrand.com/jp/ntbinormdist/)
