...
[切断正規分布](../../../05-gallery-of-distributions/truncated-normal-distribution.mdx)に従う疑似乱数を返します。乱数は超長周期 ($2^{19937}-1$)、 かつ高次元均等性（623次元）を備えた[Mersenne Twister アルゴリズム](/docs/glossary#mersenne-twister-algorithm) を基に生成されます。
...
- 乱数$x$ は、$(0,1)$ の一様乱数$U$から、
  ...
  として計算されます（逆関数法）。ここで、$\Phi(\cdot)$ は[標準正規分布](../../../05-gallery-of-distributions/normal-distribution-single.mdx)の[累積分布関数](/docs/glossary#cumulative-distribution-function)で、
