...
[\
ジョンソン SU 分布](../../../05-gallery-of-distributions/johnson-su-distribution.mdx)に従う疑似乱数を返します。乱数は超長周期 ($2^{19937}-1$)、 かつ高次元均等性（623次元）を備えた[Mersenne Twister アルゴリズム](/docs/glossary#mersenne-twister-algorithm) を基に生成されます。
...
- **_Use invert func_** 乱数生成法を論理値で指定します。*Use invert func* に TRUE を指定すると逆関数法が用いられ、FALSE を指定すると[極座標法（Box-Muller 法）](/docs/glossary#box-muller-method)が用いられます。
- **_Use antithetic_** [対称変量法](/docs/glossary#antithetic-variant-method)を用いるか否かを論理値で指定します。*Use antithetic* に TRUE を指定すると[対称変量法](/docs/glossary#antithetic-variant-method)が用いられ，FALSEを指定すると用いられません。
- **_Use resampling_** [2次サンプリング](/docs/glossary#quadratic-resampling)を用いるか否かを論理値で指定します。*Use resampling* に TRUE を指定すると[2次サンプリング](/docs/glossary#antithetic-variant-method)が用いられ，FALSEを指定すると用いられません。
...
- 乱数$x$ は、$(0,1)$ の一様乱数$U$から、
  ...
  として計算されます（逆関数法）。ここで、$\Phi(\cdot)$は[標準正規分布](../../../05-gallery-of-distributions/normal-distribution-single.mdx)の[累積分布関数](/docs/glossary#cumulative-distribution-function)です。
